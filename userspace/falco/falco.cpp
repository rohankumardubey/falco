...
int falco_init(int argc, char **argv)
{
	...
	try
	{
		...
		{
			throw std::runtime_error("Could not find configuration file at " + app.options().conf_filename);
		}
		...
		if(app.options().list_all_fields)
		{
			std::string all_sources = "";
			list_source_fields(engine, app.options().verbose, app.options().names_only, all_sources);
			return EXIT_SUCCESS;
		}

		if(app.options().list_source_fields != "none")
		{
		...
		if(!app.options().k8s_api.empty())
		{
			...
			// owns the pointers.
			std::string *k8s_api_ptr = new string(app.options().k8s_api);
		...
		else if(char* k8s_api_env = getenv("FALCO_K8S_API"))
		{
			...
			// and pass to inspector. The inspector then
			// owns the pointers.
			std::string *k8s_api_ptr = new string(k8s_api_env);
			std::string *k8s_api_cert_ptr = new string(app.options().k8s_api_cert);
			std::string *k8s_node_name_ptr = new string(app.options().k8s_node_name);
