...
cmdline_options::cmdline_options()
	: event_buffer_format(sinsp_evt::PF_NORMAL),
	  list_plugins(false),
	  m_cmdline_opts("Falco", "Cloud Native Runtime Security")
{
	define();
}
...
bool cmdline_options::parse(int argc, char **argv, std::string &errstr)
{
	...
	{
		errstr = std::string("Could not parse command line arguments: ") + e.what();
		return false;
	}
...
void cmdline_options::define()
{
	m_cmdline_opts.add_options()
		...
		("list",                          "List all defined fields.", cxxopts::value(list_all_fields))
		("list-source",                   "List all defined fields for the provided source. Current values for <source> are \"syscall\", \"k8s_audit\", or any source from a configured source plugin", cxxopts::value(list_source_fields)->default_value("none"), "<source>")
		...
		("o,option",                      "Set the value of option <key> to <val>. Overrides values in configuration file. <key> can be a two-part <key>.<subkey>", cxxopts::value(cmdline_config_options), "(<key>=<val>|<key.subkey>=<val>)")
